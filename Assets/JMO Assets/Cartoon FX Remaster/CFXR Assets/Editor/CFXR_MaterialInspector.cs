...
namespace CartoonFX
{
	public class MaterialInspector : ShaderGUI
	{
				...
				if(materials.Count > 0 && materials[0].shader != null)
				{
					...
					foreach(var l in lines)
					{
						...
						if(insideProperties)
						{
							...
							{
								var command = line.Substring(kGuiCommandPrefix.Length).TrimStart();
								//space
		...
		public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
		{
			...
			var force = (shaderImporter != null && shaderImporter.assetTimeStamp != lastTimestamp);
		...
		virtual protected void DisplayProperty(MaterialProperty property, MaterialEditor materialEditor)
		{
			...
			var controlRect = EditorGUILayout.GetControlRect(true, propertyHeight, EditorStyles.layerMaskField, new GUILayoutOption[0]);
	...
	internal class MaterialKeywordEnumNoPrefixDrawer : MaterialPropertyDrawer
	{
        ...
        private void SetKeyword(MaterialProperty prop, int index)
		{
			for (var i = 0; i < keywords.Length; ++i)
			{
				var keyword = GetKeywordName(prop.name, keywords[i]);
	...
	internal class GC_IfKeyword : GUICommand
	{
		...
		public override void OnGUI()
		{
			var show = ExpressionParser.EvaluateExpression(expression, (string s) =>
			{
	...
	internal class GC_IfProperty : GUICommand
	{
		...
		public override void OnGUI()
		{
			var show = ExpressionParser.EvaluateExpression(expression, EvaluatePropertyExpression);
			MaterialInspector.PushShowProperty(show);
		}

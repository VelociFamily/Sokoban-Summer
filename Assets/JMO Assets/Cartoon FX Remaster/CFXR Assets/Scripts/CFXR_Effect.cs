				...
				public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
				{
					...
					if (animateIntensity.boolValue)
					{
						startIndent(ref rect);
						{
							...
							var val = EditorGUI.Popup(rect, IntensityModeLabel, perlinIntensity.boolValue ? 1 : 0, ModePopupLabels); rect.y += propSpace;
					...
					if (animateRange.boolValue)
					{
						startIndent(ref rect);
						{
							...
							var val = EditorGUI.Popup(rect, RangeModeLabel, perlinRange.boolValue ? 1 : 0, ModePopupLabels); rect.y += propSpace;
					...
					if (animateColor.boolValue)
					{
						startIndent(ref rect);
						{
							...
							var val = EditorGUI.Popup(rect, ColorModeLabel, perlinColor.boolValue ? 1 : 0, ModePopupLabels); rect.y += propSpace;
